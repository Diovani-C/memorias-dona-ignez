...
#
# Requirements: podman, vips, bc, etc.
...
run_denoiser() {
  ...
  sudo podman run --gpus all -it \
    ...
    python run.py -opt CascadedGaze-SIDD.yml
...
run_colorization() {
  ...
  sudo podman run --gpus all -it \
    ...
    python run.py --model_path pretrain/ddcolor_modelscope.pth --input /app/input --output /app/output
...
run_superresolution() {
  ...
  sudo podman run --gpus all -it \
    ...
    python run.py -i /app/input/ -o /app/output/ --scale 2 --task classical --cpu
...
execute_action() {
  ...
  4) run_preprocessing "$(pwd)/input" "$(pwd)/output" ;;
...
execute_chain_actions() {
  ...
  echo "Available actions: 1-Denoiser, 2-Colorization, 3-Deblur, 4-Image Preprocessing, 5-Super Resolution"
  read -rp "Enter your sequence: " -a chain_actions
  ...
  for action in "${chain_actions[@]}"; do
    ...
    4) run_preprocessing "$prev_dir" "$current_dir" ;;
    5) run_superresolution "$prev_dir" "$current_dir" ;;
    *)
      echo "Invalid action in chain: $action" >&2
      ;;
    esac
