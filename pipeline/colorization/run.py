...
class ImageColorizationPipeline:
    def __init__(
        ...
        self.model.load_state_dict(
            torch.load(model_path, map_location="cpu")["params"], strict=False
        )
        self.model.eval()
    ...
    def process(self, img):
        ...
        output_ab = self.model(
            tensor_gray_rgb
        ).cpu()  # (1, 2, self.input_size, self.input_size)
...
def main():
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="pretrain/ddcolor_modelscope.pth",
        help="Path to the model weights",
    )
    parser.add_argument(
        "--input", type=str, default="input/", help="Input image folder"
    )
    parser.add_argument("--output", type=str, default="output", help="Output folder")
    parser.add_argument(
        "--input_size", type=int, default=512, help="Input size for the model"
    )
    parser.add_argument(
        "--model_size",
        type=str,
        default="large",
        help="DDColor model size (tiny or large)",
    )

    parser.add_argument("--cpu", action="store_true", help="Force to use CPU")
    parser.add_argument(
        "--compile", action="store_true", help="Run torch.compile on the model"
    )
